...
MovieReader::MovieReader(const string &filename_, unsigned int width_,
  ...
  avpicture_fill((AVPicture *)pFrameRGB, buffer, AV_PIX_FMT_BGR24, width,
                 height);
}
...
bool MovieReader::getFrame(vector<uint8_t> &pixels) {
  while (1) {
    ...
    SwsContext *swsCtx;
    swsCtx = sws_getCachedContext(NULL, c->width, c->height, c->pix_fmt, width,
                                  height, AV_PIX_FMT_BGR24, SWS_BICUBIC, NULL,
                                  NULL, NULL);
    sws_scale(swsCtx, ((AVPicture *)pFrame)->data,
              ((AVPicture *)pFrame)->linesize, 0, c->height,
    ...
    for (unsigned int y = 0; y < height; y++) {
      for (unsigned int x = 0; x < width; x++) {
        ...
        pixels[y * 4 * width + 4 * x + 0] =
            pFrameRGB->data[0][y * pFrameRGB->linesize[0] + 3 * x + 0];
        pixels[y * 4 * width + 4 * x + 1] =
            pFrameRGB->data[0][y * pFrameRGB->linesize[0] + 3 * x + 1];
        pixels[y * 4 * width + 4 * x + 2] =
            pFrameRGB->data[0][y * pFrameRGB->linesize[0] + 3 * x + 2];
      }
    }
